...
test.describe('admin ↔ storefront data boundary', () => {
  ...
  test('admin catalog API never becomes a public mutation surface', async ({ request }) => {
    ...
    expect([401, 403, 409]).toContain(response.status()); expect(response.status()).not.toBe(201);
  });
  ...
  test('public product pages remain backed by catalog routes', async ({ page }) => {
    await page.goto('/shop'); const link = page.locator('a[href*="/product/"]').first(); await expect(link).toBeVisible({ timeout: 10000 }); const href = await link.getAttribute('href'); expect(href).toMatch(/^\/product\/.+/); const response = await page.goto(href!); expect(response?.status()).toBeLessThan(500); await expect(page.locator('body')).not.toContainText('Application error'); await expect(page.getByRole('button', { name: /Add to Cart/i }).first()).toBeVisible({ timeout: 10000 });
  });

  test('public product page does not expose admin management UI', async ({ page }) => {
    await page.goto('/shop'); const link = page.locator('a[href*="/product/"]').first(); await expect(link).toBeVisible({ timeout: 10000 }); await link.click(); await expect(page.locator('body')).not.toContainText(/Product command center|Catalog workspace|Commerce Control/i);
  });

  test('admin catalog is protected while storefront remains public', async ({ page }) => {
    const storefront = await page.goto('/shop', { waitUntil: 'domcontentloaded' }); expect(storefront?.status()).toBeLessThan(500); await expect(page.locator('body')).not.toContainText('Application error'); await page.goto('/admin/products', { waitUntil: 'domcontentloaded' }); await expect(page).toHaveURL(/admin\/login/);
  });
});
...
test.describe('responsive catalog surfaces', () => {
  ...
  test('mobile storefront catalog has no document overflow', async ({ page }) => { await page.goto('/shop', { waitUntil: 'domcontentloaded' }); const dimensions = await page.evaluate(() => ({ width: document.documentElement.scrollWidth, viewport: window.innerWidth })); expect(dimensions.width).toBeLessThanOrEqual(dimensions.viewport + 2); await expect(page.locator('body')).not.toContainText('Application error'); });
  test('mobile admin catalog redirects cleanly', async ({ page }) => { await page.goto('/admin/products', { waitUntil: 'domcontentloaded' }); await expect(page).toHaveURL(/admin\/login/); await expect(page.locator('body')).not.toContainText('Application error'); });
});
