...
export async function getSaleProducts(limit=8){const rows=await db.product.findMany({where:{active:true},include:productInclude,orderBy:{updatedAt:'desc'},take:limit*3});return rows.filter((p:any)=>p.salePrice<p.mrp).slice(0,limit).map(mapProduct)}
...
export async function getStorefrontProduct(slug:string){const p=await db.product.findFirst({where:{slug,active:true},include:{category:true,variants:{orderBy:{stock:'desc'}},images:{orderBy:{sortOrder:'asc'}},reviews:{where:{approved:true},select:{rating:true}}}});if(!p)return null;const variant=p.variants.find(v=>v.stock-v.reserved>0)||p.variants[0];const reviews=p.reviews;const rating=reviews.length?reviews.reduce((sum,r)=>sum+r.rating,0)/reviews.length:0;return {id:p.id,name:p.name,slug:p.slug,category:p.category.name,categorySlug:p.category.slug,price:variant?.price??p.salePrice,mrp:p.mrp,image:p.images[0]?.url||'',images:p.images.map(i=>i.url),colors:[...new Set(p.variants.map(v=>v.color))],sizes:[...new Set(p.variants.map(v=>v.size))],description:p.description,variantId:variant?.id||'',variants:p.variants.map(v=>({id:v.id,color:v.color,size:v.size,price:v.price??p.salePrice,stock:Math.max(0,v.stock-v.reserved),sku:v.sku})),rating,reviewCount:reviews.length,fabric:p.fabric,care:p.care}}
...
export async function getHomeCms(){
 // Homepage Studio owns ordering; do not require a key prefix because admins may
 // use meaningful keys such as home.hero.summer, home.banner.1 or campaign.2026.
 return db.cmsSection.findMany({where:activeWindow(new Date()),orderBy:{sortOrder:'asc'}});
}
