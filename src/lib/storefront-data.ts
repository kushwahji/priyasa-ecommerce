...
export async function getBestSellers(limit=4){
 ...
 const variants=await db.productVariant.findMany({where:{id:{in:groups.map(g=>g.variantId)}},select:{id:true,productId:true}});const productIds=[...new Set(groups.map(g=>variants.find(v=>v.id===g.variantId)?.productId).filter(Boolean) as string[])];
 const products=await getStorefrontProducts();const ranked=productIds.map(id=>products.find(p=>p.id===id)).filter(Boolean) as Product[];return ranked.slice(0,limit);
}
...
export async function getStorefrontProduct(slug:string){
 ...
 if(!p)return null;const variant=p.variants.find(v=>v.stock-v.reserved>0)||p.variants[0];const reviews=p.reviews;const rating=reviews.length?reviews.reduce((s,r)=>s+r.rating,0)/reviews.length:0;
