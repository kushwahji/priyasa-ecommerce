...
export async function getStorefrontProducts(options:{categorySlug?:string;limit?:number}={}):Promise<Product[]>{
 ...
 return rows.map((p):Product=>{const variant=p.variants.find(v=>v.stock-v.reserved>0)||p.variants[0];return {id:p.id,name:p.name,slug:p.slug,category:p.category.name,categorySlug:p.category.slug,price:variant?.price??p.salePrice,mrp:p.mrp,image:p.images[0]?.url||'',images:p.images.map(i=>i.url),colors:[...new Set(p.variants.map(v=>v.color))],sizes:[...new Set(p.variants.map(v=>v.size))],description:p.description,variantId:variant?.id||'',badge:p.mrp>p.salePrice?`${Math.round((1-p.salePrice/p.mrp)*100)}% OFF`:undefined};});
}
...
export async function getBestSellers(limit=4){
 ...
 if(groups.length===0)return getStorefrontProducts({limit});
