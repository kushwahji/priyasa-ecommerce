...
function templateValue(key: string, order: any, payload: any) {
  ...
  const values: Record<string, unknown> = { orderId: order?.id, orderNumber: order?.orderNumber, status: payload?.status, previousStatus: payload?.previousStatus, customerName: order?.user?.name || '', customerPhone: order?.user?.phone || '', total: order?.total, trackingNumber: order?.shipment?.trackingNumber || '', trackingUrl: order?.shipment?.trackingUrl || '' };
...
export async function triggerAutomationEvent(trigger: string, payload: any) {
  ...
  for (const rule of rules) { if (!matches(rule.conditions, payload)) continue; await db.automationRun.create({ data: { automationId: rule.id, status: 'PENDING', payload } }); }
}
export async function processAutomationRuns(limit = 25) {
  ...
  for (const run of runs) {
    try {
      ...
      await db.automationRun.update({ where: { id: run.id }, data: { status: 'SUCCESS', finishedAt: new Date() } }); done++;
    } catch (e) { await db.automationRun.update({ where: { id: run.id }, data: { status: 'FAILED', error: e instanceof Error ? e.message : 'Automation failed', finishedAt: new Date() } }); }
  }
  return done;
}
