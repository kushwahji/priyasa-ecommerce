...
type GraphSendResult = {
  connection: NonNullable<Awaited<ReturnType<typeof getAnyMetaWhatsAppConnection>>>;
...
async function graphSend(input: { to: string; payload: Record<string, unknown> }): Promise<GraphSendResult> {
  ...
  // Capture the narrowed value locally. The connection object type keeps
  // phoneNumberId nullable, so using the property later can reintroduce
  // `string | null` during strict TypeScript builds.
  ...
  const endpoint = `https://graph.facebook.com/${version}/${encodeURIComponent(phoneNumberId)}/messages`;
  ...
  return { connection, phoneNumberId, to, messageId, response: result as Record<string, unknown> };
}
...
export async function sendMetaWhatsAppText(input: { to: string; body: string; orderId?: string; automationRunId?: string }) {
  ...
  await markWhatsAppOutbound(result.phoneNumberId, result.to, body);
...
export async function sendMetaWhatsAppTemplate(input: { to: string; templateName: string; languageCode?: string; bodyParameters?: string[]; orderId?: string; automationRunId?: string }) {
  ...
  await markWhatsAppOutbound(result.phoneNumberId, result.to, `Template: ${templateName}`);
