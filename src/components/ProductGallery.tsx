...
function MediaImage({src,alt,className}:{src:string;alt:string;className?:string}){const[failed,setFailed]=useState(false);const resolved=mediaSrc(src);return <img src={failed?FALLBACK:resolved} alt={alt} className={className} loading="eager" decoding="async" onError={()=>setFailed(true)}/>}
...
export function ProductGallery({image,name,productId,gallery=[]}:Props){
  ...
  const[remoteImages,setRemoteImages]=useState<string[]>([]);const[active,setActive]=useState(0);const[zoom,setZoom]=useState(false);const[lightbox,setLightbox]=useState(false);
  useEffect(()=>{if(!productId)return;const usable=initialImages.some(src=>src&&src!==FALLBACK);if(usable)return;let cancelled=false;(async()=>{try{const r=await fetch(`/api/products/${encodeURIComponent(productId)}`,{cache:'no-store'});if(!r.ok)return;const body=await r.json().catch(()=>null);const data=body?.data?.product??body?.data?.data??body?.data??body?.product??body;const found=collectMedia(data);if(!cancelled)setRemoteImages([...new Set(found)]);}catch{}})();return()=>{cancelled=true}},[productId,initialImages]);
