...
function MediaImage({src,alt,className}:{src:string;alt:string;className?:string}){const[failed,setFailed]=useState(false);return <img src={failed?FALLBACK:src} alt={alt} className={className} loading="eager" decoding="async" onError={()=>setFailed(true)}/>}
...
export function ProductGallery({image,name,gallery=[]}:Props){
  ...
  return <div className="pdp-gallery premium-pdp-gallery">
    {hasMultiple&&<div className="pdp-thumbs">{images.map((item,i)=><button type="button" key={item+i} className={i===active?'active':''} onClick={()=>setActive(i)} aria-label={`View ${isVideo(item)?'video':'image'} ${i+1}`}><span className="gallery-thumb-media">{isVideo(item)?<video src={item} muted playsInline preload="metadata"/>:<MediaImage src={item} alt=""/>}</span>{isVideo(item)&&<span className="gallery-video-mark">▶</span>}</button>)}</div>}
    <div className={`pdp-main-image ${zoom?'is-zoomed':''}`}>
      {video?<video className="pdp-main-video" src={src} controls playsInline preload="metadata" aria-label={`${name} product video`}/>:<button type="button" className="gallery-image-button" onClick={()=>setLightbox(true)} aria-label="Open product image fullscreen"><MediaImage src={src} alt={name} className={zoom?'zoomed-image':''}/></button>}
    ...
    {hasMultiple&&<div className="pdp-mobile-slider">{images.map((item,i)=><button type="button" key={`m${item}${i}`} onClick={()=>setActive(i)} className={i===active?'active':''}>{isVideo(item)?<video src={item} muted playsInline preload="metadata"/>:<MediaImage src={item} alt={i===active?name:''}/>}</button>)}</div>}
    ...
    {lightbox&&<div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label="Product media viewer" onClick={()=>setLightbox(false)}><button type="button" className="gallery-lightbox-close" onClick={()=>setLightbox(false)} aria-label="Close fullscreen">×</button>{hasMultiple&&<button type="button" className="gallery-lightbox-prev" onClick={e=>{e.stopPropagation();prev()}} aria-label="Previous media">‹</button>}<div className="gallery-lightbox-content" onClick={e=>e.stopPropagation()}>{video?<video src={src} controls autoPlay playsInline/>:<MediaImage src={src} alt={name}/>}</div>{hasMultiple&&<button type="button" className="gallery-lightbox-next" onClick={e=>{e.stopPropagation();next()}} aria-label="Next media">›</button>}<div className="gallery-lightbox-count">{active+1} / {images.length}</div></div>}
  </div>
}
