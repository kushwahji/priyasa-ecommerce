...
export default function HomeReviewsDynamic({ section, reviews }: { section: any; reviews: Review[] }) {
  ...
  useEffect(() => setRemaining(visible.length), [visible.length]);
  const cards = useMemo(() => visible.slice(0, remaining), [visible, remaining]);
  const cta = section?.content?.cta || {};
  if (!visible.length) return <section className="home-api-reviews" style={{ padding: '38px 0 34px', background: '#fafafa' }}><div className="home-section-head"><div>{section.title && <h2>{clean(section.title)}</h2>}{section.subtitle && <p>{clean(section.subtitle)}</p>}</div></div><div className="home-empty-products">Be the first to share your Priyasa experience.</div></section>;
