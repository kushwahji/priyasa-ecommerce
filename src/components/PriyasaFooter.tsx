  ...
  return <>
    ...
    <footer className="priyasa-footer">
      <div className="priyasa-footer-newsletter">
        <div>
          <span className="footer-eyebrow">PRIYASA EDIT</span>
          <h2>Stay in the know.</h2>
          <p>Get new arrivals, private offers and style updates delivered to your inbox.</p>
        </div>
        <NewsletterForm/>
      </div>
      ...
      <div className="priyasa-footer-main">
        <div className="priyasa-footer-brand">
          ...
          <div className="priyasa-footer-address">
            <span>PRIYASA INDIA</span>
            <p>KM34 JP Noida, GB Nagar<br/>201304 UP, India</p>
          </div>
          <div className="priyasa-footer-socials" aria-label="Social links">
        ...
        <nav className="priyasa-footer-links" aria-label="Footer navigation">
          {groups.map(group=><details key={group.title} open>
            <summary>{group.title}<span>+</span></summary>
            <div className="priyasa-footer-link-list">
              {group.items.map(([label,href])=><Link key={href} href={href}>{label}</Link>)}
            </div>
          </details>)}
        </nav>

        <div className="priyasa-footer-app">
          <span className="footer-eyebrow">SHOP ANYWHERE</span>
          <h3>Download the Priyasa app</h3>
          <p>Faster checkout, order tracking and exclusive app-only offers.</p>
          <AppDownloadBadges/>
          <div className="footer-payment-title">Secure payments</div>
          <div className="footer-payment-row" aria-label="Payment methods">
            <span>VISA</span><span>UPI</span><span>RuPay</span><span>MC</span><span>AMEX</span>
          </div>
        </div>
      </div>

      <div className="priyasa-footer-bottom">
        <span>© {new Date().getFullYear()} PRIYASA. All Rights Reserved.</span>
