...
import {BagIcon,CloseIcon} from '@/components/StorefrontIcons';
...
export default function SmartCartExtras(){
 ...
 function changeQty(variantId:string,delta:number){persist(items.map(i=>i.variantId===variantId?{...i,quantity:Math.max(1,Math.min(20,i.quantity+delta))}:i))}
 ...
 return <>{open&&count>0&&<div className="smart-cart-drawer" role="dialog" aria-modal="true" aria-label="Shopping bag"><button className="smart-cart-backdrop" aria-label="Close shopping bag" onClick={()=>setOpen(false)}/><aside className="smart-cart-panel">
