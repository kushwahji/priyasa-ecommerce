...
export function ProductPurchase({ product }: { product: Product }) {
    ...
    fetch('/api/customer/wishlist', { cache: 'no-store' })
      ...
      .then((data) => {
        if (data.authenticated) {
          setAuth(true);
          setLiked((data.data || []).some((item: { id: string }) => item.id === product.id));
        } else {
  ...
  function selectColor(nextColor: string) {
    setColor(nextColor);
    const nextSize = sizes.find((candidate) => variants.some((item) => item.color === nextColor && item.size === candidate && Number(item.stock || 0) > 0));
  ...
  return <div className="purchase-panel premium-purchase-panel">
    <div className="purchase-heading-row">
      <div><span className="purchase-kicker">SELECT YOUR FIT</span><strong>Choose options</strong></div>
      <button type="button" className={`pdp-wishlist ${liked ? 'liked' : ''}`} onClick={toggleWishlist} aria-label={liked ? 'Remove from wishlist' : 'Add to wishlist'}>{liked ? '♥' : '♡'} <span>{liked ? 'Saved' : 'Wishlist'}</span></button>
    </div>
    ...
    {sizes.length > 0 && <div className="option-block">
      <div className="option-label"><strong>Size</strong><a className="size-guide-link" href="/size-guide">Size Guide ↗</a></div>
      <div className="size-options">{sizes.map((item) => {
        const available = !variants.length || variants.some((entry) => (!color || entry.color === color) && entry.size === item && Number(entry.stock || 0) > 0);
        return <button type="button" disabled={!available} key={item} className={size === item ? 'active' : ''} onClick={() => setSize(item)}>{item}{!available && <small>Sold out</small>}</button>;
      })}</div>
    </div>}
