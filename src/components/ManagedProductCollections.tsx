...
export default function ManagedProductCollections() {
  ...
  return <section className="managed-product-collections">
    ...
    <div className="managed-product-editor">
      <label>Rail<select value={draft.type} onChange={(e) => { const type=e.target.value as CollectionType; setDraft((d) => ({ ...d, type, key:`home.products.${type === 'products-sale' ? 'sale' : 'latest'}`, ...defaults[type] })); }}><option value="products-sale">Flash Sale</option><option value="products-latest">Latest Launch</option></select></label>
