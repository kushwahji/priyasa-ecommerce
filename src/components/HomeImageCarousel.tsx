...
type DisplayConfig = { desktop_columns?: number; tablet_columns?: number; mobile_columns?: number; desktop_cards?: number; tablet_cards?: number; mobile_cards?: number; mobile_scroll?: boolean; desktopColumns?: number; tabletColumns?: number; mobileColumns?: number; mobileScroll?: boolean };
...
export default function HomeImageCarousel({ slides, display }: { slides: Slide[]; display?: DisplayConfig }) {
 const desktop = columns(display?.desktop_columns ?? display?.desktopColumns ?? display?.desktop_cards, 4);
 const tablet = columns(display?.tablet_columns ?? display?.tabletColumns ?? display?.tablet_cards, Math.min(3, desktop));
 const mobile = columns(display?.mobile_columns ?? display?.mobileColumns ?? display?.mobile_cards, 2, 4);
