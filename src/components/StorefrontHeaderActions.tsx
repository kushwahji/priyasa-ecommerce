...
export function StorefrontHeaderActions({ searchPlaceholder = 'Search for products, brands and more...' }: Props) {
  ...
  useEffect(() => { if (panel !== 'search') return; const value = query.trim(); if (value.length < 2) { setSearchItems([]); setSearchLoading(false); setSearchError(false); return; } const requestId = ++searchRequest.current; const timer = window.setTimeout(async () => { setSearchLoading(true); setSearchError(false); try { const response = await fetch(`/api/storefront/search-advanced?q=${encodeURIComponent(value)}&limit=6`, { cache: 'no-store' }); const payload = await response.json(); if (requestId !== searchRequest.current) return; if (!response.ok) throw new Error('Search failed'); setSearchItems(Array.isArray(payload?.data?.products) ? payload.data.products : []); } catch { if (requestId === searchRequest.current) { setSearchItems([]); setSearchError(true); } } finally { if (requestId === searchRequest.current) setSearchLoading(false); } }, 180); return () => window.clearTimeout(timer); }, [query, panel]);

  const close = () => setPanel(null);
  const openSearch = () => { setPanel('search'); window.setTimeout(() => document.getElementById('header-live-search')?.focus(), 0); };
  const openLogin = () => { setPanel(null); setLoginOpen(true); };
  ...
  return <>
    <div className="actions">
      ...
      <button type="button" className="header-icon" aria-label="Login with mobile" onClick={openLogin}><UserIcon /></button>
    ...
    {panel && <>
      ...
      {panel === 'search' && <section className="e2-panel e2-panel--search" role="dialog" aria-modal="true" aria-label="Search Priyasa"><div className="e2-panel-head"><span className="e2-panel-title">Search Priyasa</span><button className="e2-panel-close" onClick={close} aria-label="Close search"><CloseIcon /></button></div><div className="e2-search-body"><form className="e2-search-form" action="/search" onSubmit={e => { if (!query.trim()) e.preventDefault(); }}><SearchIcon /><input id="header-live-search" autoFocus value={query} onChange={e => setQuery(e.target.value)} name="q" placeholder={searchPlaceholder} aria-label="Search products" autoComplete="off" />{query && <button type="button" className="e2-search-clear" onClick={() => setQuery('')} aria-label="Clear search">×</button>}<button type="submit">Search</button></form><div className="header-search-results" aria-live="polite">{!query.trim() && <><div className="e2-search-label">Trending</div><div className="e2-search-links"><Link href="/new-arrivals" onClick={close}>New Arrivals</Link><Link href="/offers" onClick={close}>Best Offers</Link><Link href="/category/dresses" onClick={close}>Dresses</Link><Link href="/category/kurta-sets" onClick={close}>Kurta Sets</Link><Link href="/category/ethnic-wear" onClick={close}>Ethnic Wear</Link></div></>}{query.trim() && searchLoading && <div className="header-search-status">Searching Priyasa…</div>}{query.trim() && !searchLoading && searchError && <div className="header-search-status">Search is temporarily unavailable. Try again.</div>}{query.trim() && !searchLoading && !searchError && searchItems.length === 0 && <div className="header-search-status">No products found for “{query.trim()}”.</div>}{query.trim() && !searchLoading && searchItems.length > 0 && <><div className="e2-search-label">Products</div><div className="header-search-product-list">{searchItems.map(product => { const discount = Number(product.mrp) > Number(product.price) ? Math.round((1 - Number(product.price) / Number(product.mrp)) * 100) : 0; return <Link key={product.id} href={`/product/${product.slug}`} className="header-search-product" onClick={close}><img src={product.image || '/images/product-placeholder.svg'} alt="" loading="lazy" /><span className="header-search-product-copy"><strong>{product.name}</strong><small>{product.category || 'Fashion'}</small><span><b>{money(product.price)}</b>{Number(product.mrp) > Number(product.price) && <del>{money(product.mrp)}</del>}{discount > 0 && <em>{discount}% OFF</em>}</span></span></Link>; })}</div><Link className="header-search-view-all" href={`/search?q=${encodeURIComponent(query.trim())}`} onClick={close}>View all results for “{query.trim()}” →</Link></>}</div></div></section>}
      {panel === 'account' && <aside className="e2-panel" role="dialog" aria-modal="true" aria-label="Account"><div className="e2-panel-head"><span className="e2-panel-title">My Priyasa Account</span><button className="e2-panel-close" onClick={close} aria-label="Close account"><CloseIcon /></button></div><div className="e2-account-body"><div className="e2-account-grid"><Link className="e2-account-link" href="/account" onClick={close}><span>Account overview</span><span>→</span></Link><Link className="e2-account-link" href="/account/orders" onClick={close}><span>My orders</span><span>→</span></Link><Link className="e2-account-link" href="/wishlist" onClick={close}><span>Wishlist</span><span>→</span></Link><Link className="e2-account-link" href="/account/returns" onClick={close}><span>Returns & exchanges</span><span>→</span></Link><Link className="e2-account-link" href="/help" onClick={close}><span>Help Centre</span><span>→</span></Link></div><button className="e2-account-login" type="button" onClick={openLogin}>Login with mobile / Create account</button></div></aside>}
