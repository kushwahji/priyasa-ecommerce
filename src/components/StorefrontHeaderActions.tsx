...
import {SearchIcon,UserIcon,HeartIcon,BagIcon,HelpIcon} from '@/components/StorefrontIcons';
...
export function StorefrontHeaderActions(){
 ...
 return <div className="actions"><Link href="/search" aria-label="Search" className="header-icon"><SearchIcon/></Link><Link href="/help" aria-label="Help Centre" className="header-icon header-help"><HelpIcon/></Link><AuthButton/><Link href="/wishlist" aria-label="Wishlist" className="header-icon"><HeartIcon/></Link><Link href="/cart" aria-label={`Shopping bag${count?`, ${count} items`:''}`} className="header-icon" data-count={count||undefined}><BagIcon/></Link></div>
}
