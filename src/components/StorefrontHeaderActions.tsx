...
import {CloseIcon,HeartIcon,BagIcon,SearchIcon,UserIcon,TruckIcon,GiftIcon,HelpIcon} from '@/components/StorefrontIcons';
...
export function StorefrontHeaderActions(){
 const[items,setItems]=useState<CartItem[]>([]);const[panel,setPanel]=useState<'search'|'account'|'cart'|null>(null);const[query,setQuery]=useState('');
 ...
 const accountLinks=[['Account overview','/account'],['My orders','/account/orders'],['Wishlist','/wishlist'],['Returns & exchanges','/account/returns'],['Help Centre','/help']] as const;
 return <>
  <div className="actions">
   <button type="button" className="header-icon" aria-label="Search" onClick={()=>setPanel('search')}><SearchIcon/></button>
   <button type="button" className="header-icon header-account-trigger" aria-label="My Account" aria-haspopup="dialog" aria-expanded={panel==='account'} onClick={()=>setPanel('account')}><UserIcon/></button>
  ...
  {panel&&<><button className="e2-panel-backdrop" aria-label="Close panel" onClick={close}/>
   {panel==='search'&&<section className="e2-panel e2-panel--search" role="dialog" aria-modal="true" aria-label="Search">
    <div className="e2-panel-head"><span className="e2-panel-title">Search</span><button className="e2-panel-close" onClick={close} aria-label="Close search"><CloseIcon/></button></div>
   ...
   {panel==='account'&&<aside className="e2-panel e2-panel--account" role="dialog" aria-modal="true" aria-label="My Account">
    <div className="e2-panel-head"><div><span className="e2-panel-kicker">PRIYASA</span><span className="e2-panel-title">My Account</span></div><button className="e2-panel-close" onClick={close} aria-label="Close account"><CloseIcon/></button></div>
    <div className="e2-account-body">
      <div className="e2-account-welcome"><div className="e2-account-avatar"><UserIcon/></div><div><strong>Hello, beautiful!</strong><span>Sign in for a more personalised Priyasa experience.</span></div><Link href="/login" onClick={close}>Login / Sign up <span>›</span></Link></div>
      <nav className="e2-account-grid" aria-label="Account menu">{accountLinks.map(([label,href])=><Link className="e2-account-link" href={href} onClick={close} key={href}><span>{label}</span><span>›</span></Link>)}</nav>
      <div className="e2-account-shortcuts"><Link href="/account/orders" onClick={close}><TruckIcon/><span>Orders</span></Link><Link href="/wishlist" onClick={close}><HeartIcon/><span>Wishlist</span></Link><Link href="/offers" onClick={close}><GiftIcon/><span>Offers</span></Link><Link href="/help" onClick={close}><HelpIcon/><span>Help</span></Link></div>
    </div>
   </aside>}
