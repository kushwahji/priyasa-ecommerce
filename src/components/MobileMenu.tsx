...
export function MobileMenu(){
 ...
 return <>
  ...
  <aside className={`mobile-drawer ${open?'is-open':''}`} aria-hidden={!open} aria-label="PRIYASA shop menu">
   <div className="mobile-drawer-head"><button type="button" className="mobile-drawer-brand" onClick={()=>navigate('/')} aria-label="PRIYASA home"><img className="mobile-drawer-brand-icon" src="/images/priyasa-icon.svg" alt="" width={46} height={46}/><span className="mobile-drawer-brand-name">PRIYASA</span><span>Every You, Beautiful.</span></button><button type="button" aria-label="Close menu" onClick={close}><CloseIcon/></button></div>
