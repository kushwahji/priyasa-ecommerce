...
import {ChevronIcon,CloseIcon,MenuIcon,SearchIcon,UserIcon,WalletIcon,TruckIcon,HelpIcon} from '@/components/StorefrontIcons';

type MenuItem=[string,string];
type MenuGroup={title:string;items:MenuItem[]};

const groups:MenuGroup[]=[
  {title:'Shop',items:[['Shop All','/shop'],['New In','/new-arrivals'],['Women','/category/women'],['Men','/category/men'],['Kids','/category/kids']]},
  ...
  {title:'Discover',items:[['Collections','/collections'],['Offers','/offers'],['Best Sellers','/shop?sort=best-selling'],['Under ₹999','/shop?sort=price-low']]},
];
  ...
  return <>
    ...
    <aside className={`mobile-drawer ${open?'is-open':''}`} aria-hidden={!open} aria-label="Priyasa shop menu">
      <div className="mobile-drawer-head">
        <Link href="/" className="mobile-drawer-brand" onClick={close} aria-label="PRIYASA home"><img src="/images/priyasa-logo.svg" alt="PRIYASA"/><span>Shop your style</span></Link>
        <button type="button" aria-label="Close menu" onClick={close}><CloseIcon/></button>
      </div>
      <div className="mobile-drawer-account">
        <div><UserIcon/><div><strong>My Priyasa</strong><span>Sign in to view your account</span></div></div>
        <Link href="/account" onClick={close}>Sign in <ChevronIcon/></Link>
      </div>
      ...
      <div className="mobile-drawer-foot">
        <Link href="/account" onClick={close}><UserIcon/><span>My Account</span></Link>
