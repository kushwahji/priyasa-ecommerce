...
export function MobileMenu(){
 ...
 return <>
  ...
  <aside className={`mobile-drawer ${open?'is-open':''}`} aria-hidden={!open} aria-label="Priyasa shop menu">
   ...
   <div className="mobile-drawer-account"><div><UserIcon/><div><strong>Hello, beautiful!</strong><span>Sign in for a more personalised Priyasa experience</span></div></div><button type="button" onClick={()=>navigate('/account')}>Login / Sign up <ChevronIcon/></button></div>
   ...
   <div className="mobile-drawer-foot"><Link href="/wishlist" onClick={close}><HeartIcon/><span>Wishlist</span></Link><Link href="/account/orders" onClick={close}><TruckIcon/><span>Orders</span></Link><Link href="/account/wallet" onClick={close}><WalletIcon/><span>Wallet</span></Link><Link href="/offers" onClick={close}><GiftIcon/><span>Offers</span></Link><Link href="/help" onClick={close}><HelpIcon/><span>Help</span></Link></div>
  </aside>
 </>;
}