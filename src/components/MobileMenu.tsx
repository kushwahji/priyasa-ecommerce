...
export function MobileMenu(){
 ...
 return <>
  ...
  <aside className={`mobile-drawer ${open?'is-open':''}`} aria-hidden={!open} aria-label="Priyasa shop menu">
   <div className="mobile-drawer-head"><button type="button" className="mobile-drawer-brand" onClick={()=>navigate('/')} aria-label="PRIYASA home"><img src="/images/priyasa-logo.svg" alt="PRIYASA"/><span>Every You, Beautiful.</span></button><button type="button" className="mobile-drawer-close" aria-label="Close menu" onClick={close}><CloseIcon/></button></div>
   ...
   <nav className="mobile-drawer-nav" aria-label="Shop menu">
    {groups.map(group=><section key={group.title} className="mobile-drawer-group"><h3>{group.title}</h3>{group.items.map(([label,href])=><button type="button" key={href} onClick={()=>navigate(href)}><span>{label}</span><ChevronIcon/></button>)}</section>)}
   </nav>
   <div className="mobile-drawer-foot" aria-label="Quick links"><button type="button" onClick={()=>navigate('/wishlist')}><HeartIcon/><span>Wishlist</span></button><button type="button" onClick={()=>navigate('/account/orders')}><TruckIcon/><span>Orders</span></button><button type="button" onClick={()=>navigate('/account/wallet')}><WalletIcon/><span>Wallet</span></button><button type="button" onClick={()=>navigate('/offers')}><GiftIcon/><span>Offers</span></button><button type="button" onClick={()=>navigate('/help')}><HelpIcon/><span>Help</span></button></div>
  </aside>
 </>;
}
