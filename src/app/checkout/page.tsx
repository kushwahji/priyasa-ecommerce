...
async function loadRazorpay(){
  ...
  await new Promise<void>((resolve,reject)=>{ const script=document.createElement('script'); script.src='https://checkout.razorpay.com/v1/checkout.js'; script.async=true; script.onload=()=>resolve(); script.onerror=()=>reject(new Error('Unable to load payment gateway. Please try again.')); document.body.appendChild(script); });
...
export default function Checkout(){
  ...
  const shipping=Number(quote?.shipping??(subtotal-discount>=999?0:99));
  ...
  return <div className="storefront-page priyasa-checkout-v3">
    ...
    <form className="checkout-v3-layout" onSubmit={submit}>
      <main className="checkout-v3-main">
        <section className="checkout-v3-summary"><div className="checkout-v3-summary-head"><h2>Order summary <span>({itemCount})</span></h2><Link href="/cart">Edit bag</Link></div><div className="checkout-v3-items">{items.slice(0,4).map(item=><div className="checkout-v3-item" key={item.variantId}>{item.image?<img src={item.image} alt=""/>:<div/>}<div><strong>{item.name}</strong><span>Qty {item.quantity}</span></div><span className="checkout-v3-item-price">{money(Number(item.price||0)*Number(item.quantity||0))}</span></div>)}{items.length>4&&<div className="checkout-v3-more">+ {items.length-4} more item{items.length-4===1?'':'s'} in your bag</div>}</div><div className="checkout-v3-totals"><div className="checkout-v3-line"><span>Subtotal</span><strong>{money(subtotal)}</strong></div><div className="checkout-v3-line discount"><span>Discount</span><strong>{discount?`−${money(discount)}`:money(0)}</strong></div><div className="checkout-v3-line"><span>Delivery</span><strong>{shipping?money(shipping):'Free'}</strong></div><div className="checkout-v3-total"><span>Total</span><strong>{money(total)}</strong></div></div><label className="checkout-v3-coupon"><input value={form.coupon} maxLength={40} placeholder="Coupon / offer code" onChange={e=>updateField('coupon',e.target.value.toUpperCase())}/><button type="button" onClick={()=>setForm(current=>({...current,coupon:current.coupon.trim().toUpperCase()}))}>Apply</button></label>{quote?.coupon&&<div className="checkout-v3-more">✓ {quote.coupon} applied successfully</div>}</section>
