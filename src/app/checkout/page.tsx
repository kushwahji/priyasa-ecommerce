...
function normalizeLocalCart(value:unknown):CartItem[]{
  ...
  for(const raw of value){
    ...
    else merged.set(variantId,{variantId,name:String(item.name||'Product'),price:Math.max(0,Number(item.price||0)),quantity,image:item.image});
  }
...
export default function Checkout(){
  ...
  const mergedAfterLogin=useRef(false);

  useEffect(()=>{
    try{setItems(normalizeLocalCart(JSON.parse(window.localStorage.getItem('priyasa_cart')||'[]')))}catch{setItems([])}
  ...
  useEffect(()=>{
    if(authenticated!==true||mergedAfterLogin.current)return;
    mergedAfterLogin.current=true;
    void (async()=>{
      let local:CartItem[]=[];
      try{local=normalizeLocalCart(JSON.parse(window.localStorage.getItem('priyasa_cart')||'[]'))}catch{}
      if(!local.length)return;
      try{
        const response=await fetch('/api/cart',{cache:'no-store'});
        const data=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(data.error||'Unable to load saved bag');
        const remote=normalizeLocalCart(data.data?.items||[]);
        const merged=new Map(remote.map(item=>[item.variantId,{...item}]));
        for(const item of local){const current=merged.get(item.variantId);if(current)current.quantity=Math.min(20,current.quantity+item.quantity);else merged.set(item.variantId,{...item})}
        const next=[...merged.values()];
        const save=await fetch('/api/cart',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:next.map(item=>({variantId:item.variantId,quantity:item.quantity})),couponCode:form.coupon.trim().toUpperCase()||undefined})});
        const saved=await save.json().catch(()=>({}));
        if(!save.ok)throw new Error(saved.error||'Unable to merge your bag');
        window.localStorage.setItem('priyasa_cart',JSON.stringify(next));
        setItems(next);
        window.dispatchEvent(new Event('priyasa-cart-updated'));
        setStatus('Your saved bag and this device bag were combined.');
      }catch(error){
        mergedAfterLogin.current=false;
        setStatus(error instanceof Error?error.message:'Unable to merge your bag. Please try again.');
      }
    })();
  },[authenticated]);

  const clientSubtotal=useMemo(()=>items.reduce((sum,item)=>sum+Math.max(0,Number(item.price||0))*Math.max(0,Number(item.quantity||0)),0),[items]);

  async function syncCartToCore(){
    if(authenticated!==true)return;
    const response=await fetch('/api/cart',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:items.map(item=>({variantId:String(item.variantId),quantity:Math.min(20,Math.max(1,Math.floor(Number(item.quantity||1))))})),couponCode:form.coupon.trim().toUpperCase()||undefined})});
    const data=await response.json().catch(()=>({}));
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(async()=>{
      ...
      if(authenticated!==true)return;
      try{const data=await getFreshQuote();if(cancelled)return;setQuote(data);if(data.codAvailable===false&&paymentMethod==='cod')setPaymentMethod('razorpay');setStatus('')}catch(error){if(cancelled)return;setQuote(null);setStatus(error instanceof Error?error.message:'Unable to calculate your order.');}
    },220);
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      const payment=await paymentResponse.json().catch(()=>({}));if(!paymentResponse.ok)throw new Error(payment.error||'Unable to start payment.');if(!payment.keyId||!payment.razorpayOrderId||Number(payment.amount)<=0)throw new Error('Payment gateway returned an invalid order. Please retry.');
      await loadRazorpay();if(!window.Razorpay)throw new Error('Payment gateway unavailable.');
      const razorpay=new window.Razorpay({key:payment.keyId,amount:payment.amount,currency:payment.currency||'INR',name:'PRIYASA',description:`Order ${payment.orderNumber||order.orderNumber}`,order_id:payment.razorpayOrderId,theme:{color:'#a81132'},handler:async(gatewayResponse:any)=>{try{setStatus('Verifying payment securely…');const verification=await fetch('/api/payments/razorpay/verify',{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':crypto.randomUUID()},body:JSON.stringify({orderId:order.orderId,razorpay_order_id:gatewayResponse?.razorpay_order_id,razorpay_payment_id:gatewayResponse?.razorpay_payment_id,razorpay_signature:gatewayResponse?.razorpay_signature})});const result=await verification.json().catch(()=>({}));if(!verification.ok)throw new Error(result.error||'Payment verification failed.');window.localStorage.removeItem('priyasa_cart');window.location.href=`/checkout/success?order=${encodeURIComponent(order.orderNumber)}`}catch(error){setBusy(false);setStatus(error instanceof Error?error.message:'Payment verification failed. Please contact support.')}},modal:{ondismiss:()=>{setBusy(false);setStatus('Payment was not completed. Your order remains available in My Orders so you can retry.')}}});
      razorpay.on('payment.failed',(failure:any)=>{setBusy(false);setStatus(failure?.error?.description||'Payment failed. Please retry.')});razorpay.open();
