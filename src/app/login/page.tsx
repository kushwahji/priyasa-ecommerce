...
export default function LoginPage(){
  ...
  useEffect(()=>{let active=true;fetch('/api/auth/session',{cache:'no-store'}).then(r=>r.json()).then(d=>{if(!active)return;if(d?.authenticated){router.replace('/account');return;}setChecking(false);setOpen(true)}).catch(()=>{if(active){setChecking(false);setOpen(true)}});return()=>{active=false}},[router]);
  if(checking)return <main className="customer-auth-loading"><BrandLogo href="/" compact/><span>Checking your Priyasa account…</span></main>;
