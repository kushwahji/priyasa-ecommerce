...
function heroFor(section:HomeSection){const items=Array.isArray(section.content?.items)?section.content.items:[];const slides=(items.length?items:[section]).map((item:any,i:number)=>({id:`${section.id}-${i}`,title:item.title||section.title,subtitle:item.subtitle||section.subtitle,imageUrl:item.image_url||item.imageUrl||image(section),mobileImageUrl:item.mobile_image_url||item.mobileImageUrl||section.mobile_image_url||section.mobileImageUrl||item.image_url||item.imageUrl||image(section),ctaLabel:item.cta_label||item.ctaLabel||section.cta_label||section.ctaLabel||'Shop Now',ctaHref:item.cta_href||item.ctaHref||section.cta_href||section.ctaHref||'/shop'}));return <section className="home-section home-hero-multi home-managed-hero" data-testid="home-hero" key={section.id}><HomeHeroCarousel slides={slides}/></section>}
async function DynamicProducts({section,rail=false,more=false,titleOverride,subtitleOverride}:{section:HomeSection;rail?:boolean;more?:boolean;titleOverride?:string;subtitleOverride?:string}){const q=section.content?.query||{};const configured=Number(q.limit);const limit=more?Math.min(Math.max(configured||30,3),30):12;const type=typeOf(section);let products:any[]=[];if(type==='new_arrivals'||type==='new-arrivals'||keyOf(section).includes('new-arrivals'))products=await getCachedStorefrontProducts({limit,sort:String(q.sort||'newest'),search:q.search?String(q.search):undefined});else if(type==='trending'||type==='best-sellers'||keyOf(section).includes('best-seller'))products=await getCachedStorefrontProducts({limit,sort:String(q.sort||'popular'),search:q.search?String(q.search):undefined});else if(type==='products-sale'||type==='sale'||keyOf(section).includes('flash-sale'))products=await getProductsForHomeSection('sale',limit);else products=await getProductsForHomeSection(type,limit);if(!products.length)return null;return <section className={`home-section home-product-section home-managed-products ${more?'home-more-products':''}`} data-testid={more?'home-best-sellers':'home-casual-products'} key={section.id}><div className="home-section-head"><div><span className="home-kicker">{text(subtitleOverride||section.subtitle)||'PRIYASA EDIT'}</span><h2>{text(titleOverride||section.title)||'Curated for you'}</h2></div><Link className="home-view-all" href={href(section)||'/shop'}>{label(section)} →</Link></div>{rail?<HomeProductRail products={products}/>:<HomeProductGrid products={products} initialVisible={Math.min(12,products.length)} step={6}/>}</section>}
...
export default async function Home(){
 ...
 if(latestSection){const r=await DynamicProducts({section:latestSection,rail:true,titleOverride:text(latestSection.title)||'Casual Styles',subtitleOverride:text(latestSection.subtitle)||'12 fresh styles picked for you'});if(r)body.push(r);}else if(latest.length)body.push(<section className="home-section home-managed-products home-product-section" data-testid="home-casual-products" key="fallback-new"><div className="home-section-head"><div><span className="home-kicker">12 fresh styles picked for you</span><h2>Casual Styles</h2></div><Link className="home-view-all" href="/shop">View All →</Link></div><HomeProductRail products={latest}/></section>);
 if(editorial)body.push(<HomeEditorialRail key={editorial.id} products={latest} title={text(editorial.title)||'Trending Now'} subtitle={text(editorial.subtitle)||'Explore the latest looks'} href={href(editorial)||'/shop'}/>);else if(latest.length)body.push(<HomeEditorialRail key="fallback-editorial" products={latest} title="Trending Now" subtitle="Six looks, one Priyasa mood"/>);
 ...
 if(bestSection){const r=await DynamicProducts({section:bestSection,more:true});if(r)body.push(r);}else if(popular.length)body.push(<section className="home-section home-managed-products home-product-section home-more-products" data-testid="home-best-sellers" key="fallback-more"><div className="home-section-head"><div><span className="home-kicker">Most loved by shoppers</span><h2>Best Sellers</h2></div><Link className="home-view-all" href="/shop">View All →</Link></div><HomeProductGrid products={popular} initialVisible={12} step={6}/></section>);
