...
export default async function Home(){
 const [fallbackBest,categories,sections,latest,saleProducts]=await Promise.all([getBestSellers(4),getStorefrontCategories(),getHomeCms(),getLatestLaunches(8),getProductsForHomeSection('products-sale',8)]);
 const festival=festivalSlug(categories);const festivalProducts=festival?await getStorefrontProducts({categorySlug:festival,limit:8}):[];
 ...
 return <>
  ...
  {!hasCmsProducts&&latest.length>0&&<section className="section section-tight home-product-section"><div className="section-head"><div><span className="eyebrow dark">JUST DROPPED</span><h2>Latest Collection</h2><p className="section-subtitle">Fresh styles, newly added to Priyasa.</p></div><Link className="text-link" href="/new-arrivals">View All →</Link></div><div className="product-grid product-grid-editorial">{latest.map(p=><ProductCard key={p.id} product={p}/>)}</div></section>}
  {!hasCmsProducts&&fallbackBest.length>0&&<section className="section section-tight home-product-section"><div className="section-head"><div><span className="eyebrow dark">MOST LOVED</span><h2>Best Sellers</h2><p className="section-subtitle">The pieces customers keep coming back for.</p></div><Link className="text-link" href="/shop">View All →</Link></div><div className="product-grid product-grid-editorial">{fallbackBest.map(p=><ProductCard key={p.id} product={p}/>)}</div></section>}
  {!hasCmsProducts&&festivalProducts.length>0&&<section className="section section-tight home-product-section festival-home-section"><div className="section-head"><div><span className="eyebrow dark">CELEBRATE IN STYLE</span><h2>Festival Collection</h2><p className="section-subtitle">Occasion-ready pieces for the season.</p></div><Link className="text-link" href={`/category/${festival}`}>Shop Collection →</Link></div><div className="product-grid product-grid-editorial">{festivalProducts.map(p=><ProductCard key={p.id} product={p}/>)}</div></section>}
  {!hasCmsProducts&&saleProducts.length>0&&<section className="section section-tight home-product-section"><div className="section-head"><div><span className="eyebrow dark">SPECIAL EDIT</span><h2>Sale Favourites</h2><p className="section-subtitle">Beautiful styles, now at a better value.</p></div><Link className="text-link" href="/offers">View Offers →</Link></div><div className="product-grid product-grid-editorial">{saleProducts.map(p=><ProductCard key={p.id} product={p}/>)}</div></section>}
  {!hasCmsCategoryGrid&&categories.length>0&&<section className="section section-tight home-categories"><div className="section-head"><div><span className="eyebrow dark">DISCOVER YOUR STYLE</span><h2>Shop by Category</h2></div><Link className="text-link" href="/shop">View All →</Link></div><div className="category-grid category-grid-editorial">{categories.slice(0,6).map(c=><Link className="category-card" key={c.id} href={`/category/${c.slug}`}><div className="category-image" style={{backgroundImage:`url(${c.imageUrl||''})`}}/><div className="category-label"><strong>{c.name}</strong><span>Shop Now →</span></div></Link>)}</div></section>}
