  ...
  return (
    <div className={styles.shell}>
      ...
      <main className={styles.main}>
        <header className={styles.top}><input className={styles.search} placeholder="Search the control center…" aria-label="Search control center"/><span className={styles.topSpacer}/><Link className={styles.topLink} href="/admin/orders">Orders</Link><Link className={styles.topLink} href="/admin/customers">Customers</Link><span className={styles.avatar}>A</span></header>
        <div className={styles.content}>
          ...
          <section className={styles.section}><div className={styles.panel}><div className={styles.panelHead}><div><h3>Return requests</h3><span>Approve, schedule, inspect and progress each request.</span></div><span className={styles.pill}>{returns.length} records</span></div><div style={{overflowX:'auto'}}><table className={styles.table}><thead><tr><th>Order</th><th>Customer</th><th>Reason</th><th>Status</th><th>Amount</th><th>Action</th></tr></thead><tbody>{returns.map(r => <tr key={r.id}><td><Link href={`/admin/orders/${r.orderId}`}><strong>{r.order.orderNumber}</strong></Link></td><td>{r.order.user?.name || r.order.user?.phone || 'Guest'}</td><td style={{maxWidth:280,whiteSpace:'normal'}}>{r.reason}</td><td><span className={styles.pill}>{r.status}</span></td><td>{money(r.amount)}</td><td><AdminReturnActions id={r.id} status={r.status} amount={r.amount} total={r.order.total}/></td></tr>)}</tbody></table></div>{!returns.length&&<div className="empty">No return requests yet. Customer return requests will appear here.</div>}</div></section>
          <section className={styles.section}><div className={styles.panel}><div className={styles.panelHead}><div><h3>Refund ledger</h3><span>Provider references and refund status for financial traceability.</span></div><span className={styles.pill}>{refunds.length} records</span></div><div style={{overflowX:'auto'}}><table className={styles.table}><thead><tr><th>Order</th><th>Amount</th><th>Provider refund</th><th>Status</th><th>Created</th></tr></thead><tbody>{refunds.map(r => <tr key={r.id}><td>{r.payment.order.orderNumber}</td><td>{money(r.amount)}</td><td>{r.providerRefundId || '—'}</td><td><span className={styles.pill}>{r.status}</span></td><td>{r.createdAt.toLocaleDateString('en-IN')}</td></tr>)}</tbody></table></div>{!refunds.length&&<div className="empty">No refunds recorded yet.</div>}</div></section>
