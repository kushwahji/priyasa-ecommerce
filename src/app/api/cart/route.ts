...
function mapUpstreamCart(data: any): Cart | null {
  ...
  return {
    id: String(cart.id),
    couponCode: cart.coupon_code || cart.couponCode || null,
    items: Array.isArray(cart.items) ? cart.items.map((i: any): CartItem => ({
      id: String(i.id),
      variantId: String(i.variant_id ?? i.variantId ?? i.variant?.id ?? ''),
      ...
      image: i.variant?.image_url || i.variant?.product?.media?.[0]?.url || '',
    })).filter((i: CartItem) => i.variantId && i.quantity > 0) : [],
  };
}

async function identity() {
  const c = await cookies();
  const userId = c.get('priyasa_local_user_id')?.value;
  const existingSessionKey = c.get('priyasa_cart_session')?.value;
  const sessionKey = existingSessionKey || crypto.randomUUID();
  return { userId, sessionKey, isNewSession: !userId && !existingSessionKey };
}

async function guestGet(identityValue: Awaited<ReturnType<typeof identity>>) {
  const { userId, sessionKey } = identityValue;
  const cart = await db.cart.findFirst({
    where: userId ? { userId } : { sessionKey },
    include: { items: { include: { variant: { include: { product: { include: { images: { orderBy: { sortOrder: 'asc' }, take: 1 } } } } } } } },
  });
  return cart ? {
    id: cart.id,
    ...
    items: cart.items.map(i => ({
      ...
      price: i.variant.price ?? i.variant.product.salePrice,
      quantity: i.quantity,
...
function withGuestCookie(response: NextResponse, identityValue: Awaited<ReturnType<typeof identity>>) {
  if (identityValue.isNewSession) {
    response.cookies.set('priyasa_cart_session', identityValue.sessionKey, {
      httpOnly: true,
      sameSite: 'lax',
...
export async function GET() {
  ...
  try {
    if (token) {
      const { response, body: result } = await priyasaApi('/api/v1/storefront/cart', { headers: authHeaders(token) });
      if (!response.ok) return NextResponse.json({ error: apiError(result, 'Unable to load cart') }, { status: response.status });
      return NextResponse.json({ data: mapUpstreamCart(result) });
    }

    const identityValue = await identity();
    const response = NextResponse.json({ data: await guestGet(identityValue) });
    return withGuestCookie(response, identityValue);
  } catch (error) {
    return NextResponse.json({ error: apiError(error, 'Unable to load cart') }, { status: 502 });
  }
}
...
export async function PUT(req: Request) {
  ...
  try {
    if (token) {
      ...
      const current = mapUpstreamCart(currentResponse.body) || ({ id: '', couponCode: null, items: [] } as Cart);
      const target = parsed.data.items;
      const currentByVariant = new Map(current.items.map((i: CartItem) => [String(i.variantId), i]));
    ...
    for (const i of parsed.data.items) {
      ...
      if (v.stock - v.reserved < i.quantity) return NextResponse.json({ error: `${v.product.name} has only ${Math.max(0, v.stock - v.reserved)} available.` }, { status: 409 });
    }

    let cart = await db.cart.findFirst({ where: userId ? { userId } : { sessionKey } });
    if (cart) cart = await db.cart.update({ where: { id: cart.id }, data: { couponCode: parsed.data.couponCode || null } });
    else cart = await db.cart.create({ data: { ...(userId ? { userId } : { sessionKey }), couponCode: parsed.data.couponCode || null } });
    ...
    return NextResponse.json({ error: apiError(error, 'Unable to save cart') }, { status: 502 });
  }
}
