...
function mapUpstreamCart(data: any) {
  ...
  return cart ? { id: cart.id, couponCode: cart.coupon_code || cart.couponCode || null, items: (cart.items || []).map((i: any) => ({ variantId: String(i.variant_id ?? i.variantId ?? i.variant?.id), productId: String(i.variant?.product_id ?? i.variant?.product?.id ?? i.product_id ?? ''), name: i.variant?.product?.name || i.product?.name || 'Product', price: Number(i.unit_price ?? i.variant?.price ?? i.variant?.product?.price ?? 0), quantity: Number(i.quantity || 0), image: i.variant?.image_url || i.variant?.product?.media?.[0]?.url || '' })) } : null;
}

async function identity() {
  const c = await cookies();
  const userId = c.get('priyasa_local_user_id')?.value;
  let sessionKey = c.get('priyasa_cart_session')?.value;
  if (!userId && !sessionKey) sessionKey = crypto.randomUUID();
  return { userId, sessionKey };
}

async function guestGet() {
  const { userId, sessionKey } = await identity();
  const cart = await db.cart.findFirst({ where: userId ? { userId } : { sessionKey }, include: { items: { include: { variant: { include: { product: { include: { images: { orderBy: { sortOrder: 'asc' }, take: 1 } } } } } } } } });
  return cart ? { id: cart.id, couponCode: cart.couponCode, items: cart.items.map(i => ({ variantId: i.variantId, productId: i.variant.productId, name: i.variant.product.name, price: i.variant.price ?? i.variant.product.salePrice, quantity: i.quantity, image: i.variant.product.images[0]?.url || '' })) } : null;
}
...
export async function GET() {
  ...
  try {
    ...
    const data = await guestGet();
    const response = NextResponse.json({ data });
    const c = await cookies();
    const sessionKey = c.get('priyasa_cart_session')?.value;
    if (sessionKey && !c.get('priyasa_cart_session')) response.cookies.set('priyasa_cart_session', sessionKey, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/', maxAge: 60 * 60 * 24 * 180 });
    return response;
  ...
  try {
    if (token) {
      ...
      for (const next of target) {
        ...
        const request = { headers: authHeaders(token) };
        const result = existing
          ? await priyasaApi(`/api/v1/storefront/cart/items/${encodeURIComponent(existing.id || '')}`, { ...request, method: 'PATCH', body: JSON.stringify({ quantity: next.quantity }) })
          : await priyasaApi('/api/v1/storefront/cart/items', { ...request, method: 'POST', body: JSON.stringify({ variant_id: Number(next.variantId), quantity: next.quantity }) });
      ...
      for (const existing of current.items) {
        if (!targetByVariant.has(String(existing.variantId))) {
