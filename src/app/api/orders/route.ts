    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (data.coupon) {
        ...
        if (coupon && coupon.active && now >= coupon.startsAt && now <= coupon.endsAt && subtotal >= coupon.minCart && (!coupon.maxUses || coupon.usedCount < coupon.maxUses)) {
          couponDiscount = coupon.type === 'PERCENTAGE' ? Math.floor(subtotal * coupon.value / 100) : Math.min(coupon.value, subtotal);
      ...
      const shipping = merchandiseAfterCoupon >= 999 ? 0 : 99;
      const onlinePaymentDiscount = data.paymentMethod === 'razorpay' ? Math.min(ONLINE_PAYMENT_DISCOUNT, merchandiseAfterCoupon) : 0;
      const discount = couponDiscount + onlinePaymentDiscount;
      const total = Math.max(0, merchandiseAfterCoupon - onlinePaymentDiscount) + shipping;
      ...
      const order = await tx.order.create({
        data: {
          ...
          status: codPayment || walletPayment ? 'CONFIRMED' : 'PAYMENT_PENDING', fulfillmentStatus: 'UNFULFILLED',
          subtotal, discount, shipping, tax: 0, total, couponCode,
      ...
      const response = { orderId: order.id, orderNumber: order.orderNumber, total: order.total, paymentId: order.payment?.id, paymentMethod: data.paymentMethod, status: order.status, onlinePaymentDiscount };
