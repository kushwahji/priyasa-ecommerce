...
async function proxy(path: string, method: string, token: string, body?: unknown) {
  const { response, body: result } = await priyasaApi(path, {
    method,
...
export async function GET() {
  ...
  return proxy(endpoint, 'GET', token);
}

export async function POST(req: Request) {
  const token = await authToken();
  if (!token) return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
  return proxy(endpoint, 'POST', token, await req.json().catch(() => null));
}
...
export async function PATCH(req: Request) {
  ...
  return proxy(`${endpoint}/${encodeURIComponent(id)}`, 'PATCH', token, payload);
}
