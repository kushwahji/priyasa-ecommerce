...
export async function POST(req:Request){
 ...
 const payment=await db.payment.findUnique({where:{orderId},include:{order:{include:{items:true}}}});
 ...
 await db.$transaction(async tx=>{
  const fresh=await tx.payment.findUnique({where:{id:payment.id}});if(!fresh)throw new Error('PAYMENT_NOT_FOUND');if(fresh.status==='CAPTURED')return;
  for(const item of payment.order.items){
   ...
   await tx.inventoryMovement.create({data:{variantId:item.variantId,quantity:-item.quantity,reason:'SALE',referenceId:payment.orderId}});
  }
  await tx.payment.update({where:{id:payment.id},data:{providerPaymentId:razorpay_payment_id,status:'CAPTURED',signatureVerified:true}});
  await tx.paymentEvent.create({data:{provider:'razorpay',eventId:`verify_${razorpay_payment_id}`,eventType:'payment.verify',payload:body,paymentId:payment.id}}).catch(()=>undefined);
 });
